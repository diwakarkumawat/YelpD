...
class SearchFilter: NSObject {
    ...
    init(name: String, expanded: Bool, options: [SearchOption]) {
        self.name = name
    ...
    class func createFilters() -> [SearchFilter] {
        return [
            SearchFilter(name: "Offering a Deal", expanded: false, options: [
                SearchOption(value: nil, title: "Offering a Deal"),
                ]),
            SearchFilter(name: "Distance", expanded: true, options: [
            ...
            SearchFilter(name: "Sort By", expanded: true, options: [
            ...
            SearchFilter(name: "Category", expanded: false, options:
                yelpCategories()
            ),
        ];
    }
    ...
    class func yelpCategories() -> [SearchOption] {
        ...
        if let url = Bundle.main.url(forResource: "categories", withExtension: "json") {
            ...
            if let json = try! JSONSerialization.jsonObject(with: data, options:[]) as? [NSDictionary] {
                for dict in json {
                    let option = SearchOption(category: dict as NSDictionary)
